...
def onSelect(self, event):
    # callint the event is the self.lstList1 widget
    ...
    with conn:
        ...
        # returns a tuple, slice into 4 parts using data[] during iteration
        for data in varBody:
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # removes any blank spaces before and after user entry
    var_lname = var_lname.strip() # ensures that the first character in each word is capitalized
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0):
        ...
        with conn:
            cursor = conn.cursor()
            # check database for existence of fullname, if so, alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count

            # if chkName == 0, fullname does not exist, proceed to add data
            if chkName == 0:
                ...
                onClear(self) # clear all the textboxes
        conn.commit()
        conn.close()
    else:
    ...
    with conn:
        ...
        if count > 1:
            ...
            if confirm:
                ...
                onDeleted(self) # clear all textboxes and the selected index of lsitbox onRefresh(self) #update the listbox of the changes
                conn.commit()
            else:
...
def onDeleted(self):
    # clear the text in the following textboxes
    self.txt_fname.delete(0,END)
    self.txt_lname.delete(0,END)
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
...
def onUpdate(self):
    try:
        ...
        var_value = self.lstList2.get(var_select) # list selection's text value
    except:
    ...
    if(len(var_phone) > 0) and (len(var_email) > 0): # data is present
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cur = conn.cursor()
            # count records to see if the user's changes are already in the database, if yes, nothing to update
            ...
            # if changes are not already present in database, continue
